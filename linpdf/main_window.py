...
from linpdf.utils.icons import load_icon, icon_path
...
class MainWindow(QMainWindow):
    ...
    def _create_ribbon(self):
        ...
        self._btn_open = file_panel.add_button("Open", icon=load_icon("file-open"), tooltip="Open PDF (Ctrl+O)")
        self._btn_save = file_panel.add_button("Save", icon=load_icon("save"), tooltip="Save PDF (Ctrl+S)")
        self._btn_save_as = file_panel.add_button("Save As", icon=load_icon("save-all"), tooltip="Save as new file")
        self._btn_print = file_panel.add_button("Print", icon=load_icon("printer"), tooltip="Print PDF (Ctrl+P)")

        edit_panel = home.add_panel("Edit")
        self._btn_undo = edit_panel.add_button("Undo", icon=load_icon("undo"), tooltip="Undo (Ctrl+Z)")
        self._btn_redo = edit_panel.add_button("Redo", icon=load_icon("redo"), tooltip="Redo (Ctrl+Y)")
        edit_panel.add_separator()
        self._btn_select = edit_panel.add_button("Select", icon=load_icon("mouse-pointer"), tooltip="Select text/objects", checkable=True)
        self._btn_pan = edit_panel.add_button("Pan", icon=load_icon("hand"), tooltip="Pan mode", checkable=True)
        self._btn_select.setChecked(True)

        annot_panel = home.add_panel("Annotate")
        self._btn_highlight = annot_panel.add_button("Highlight", icon=load_icon("highlighter"), tooltip="Highlight text")
        self._btn_underline = annot_panel.add_button("Underline", icon=load_icon("underline"), tooltip="Underline text")
        self._btn_strikeout = annot_panel.add_button("Strikeout", icon=load_icon("strikethrough"), tooltip="Strikeout text")
        self._btn_note = annot_panel.add_button("Note", icon=load_icon("sticky-note"), tooltip="Add sticky note")

        view_panel = home.add_panel("View")
        self._btn_zoom_in = view_panel.add_button("Zoom In", icon=load_icon("zoom-in"), tooltip="Zoom in (Ctrl++)")
        self._btn_zoom_out = view_panel.add_button("Zoom Out", icon=load_icon("zoom-out"), tooltip="Zoom out (Ctrl+-)")
        self._btn_fit_width = view_panel.add_button("Fit Width", icon=load_icon("maximize"), tooltip="Fit page width")
        self._btn_fit_page = view_panel.add_button("Fit Page", icon=load_icon("scan"), tooltip="Fit whole page")

        insert = self._ribbon.add_tab("Insert")
        pages_panel = insert.add_panel("Pages")
        self._btn_blank_page = pages_panel.add_button("Blank Page", icon=load_icon("file-plus"), tooltip="Insert blank page")
        self._btn_from_file = pages_panel.add_button("From File", icon=load_icon("file-input"), tooltip="Insert pages from another PDF")
        insert_panel = insert.add_panel("Insert")
        self._btn_insert_image = insert_panel.add_button("Image", icon=load_icon("image"), tooltip="Insert image")
        self._btn_link = insert_panel.add_button("Link", icon=load_icon("link"), tooltip="Insert hyperlink")
        stamp_panel = insert.add_panel("Stamps")
        self._btn_stamp = stamp_panel.add_button("Stamp", icon=load_icon("stamp"), tooltip="Add stamp")

        page_tab = self._ribbon.add_tab("Page Layout")
        org_panel = page_tab.add_panel("Organize")
        self._btn_rotate_cw = org_panel.add_button("Rotate CW", icon=load_icon("rotate-cw"), tooltip="Rotate page clockwise")
        self._btn_rotate_ccw = org_panel.add_button("Rotate CCW", icon=load_icon("rotate-ccw"), tooltip="Rotate counter-clockwise")
        self._btn_delete_page = org_panel.add_button("Delete", icon=load_icon("trash-2"), tooltip="Delete current page")
        self._btn_extract = org_panel.add_button("Extract", icon=load_icon("file-output"), tooltip="Extract current page")

        review = self._ribbon.add_tab("Review")
        protect_panel = review.add_panel("Protect")
        self._btn_encrypt = protect_panel.add_button("Encrypt", icon=load_icon("lock"), tooltip="Password protect PDF")
        self._btn_sign = protect_panel.add_button("Sign", icon=load_icon("pen-sign"), tooltip="Digitally sign")
        self._btn_redact = protect_panel.add_button("Redact", icon=load_icon("eraser"), tooltip="Redact sensitive content")

        convert = self._ribbon.add_tab("Convert")
        export_panel = convert.add_panel("Export To")
        self._btn_to_word = export_panel.add_button("Word", icon=load_icon("file-text"), tooltip="Export to Microsoft Word")
        self._btn_to_excel = export_panel.add_button("Excel", icon=load_icon("file-spreadsheet"), tooltip="Export to Microsoft Excel")
        self._btn_to_ppt = export_panel.add_button("PowerPoint", icon=load_icon("presentation"), tooltip="Export to PowerPoint")
        export_panel.add_separator()
        self._btn_to_image = export_panel.add_button("Image", icon=load_icon("image-down"), tooltip="Export as images")
        ocr_panel = convert.add_panel("OCR")
        self._btn_ocr = ocr_panel.add_button("OCR", icon=load_icon("scan-text"), tooltip="Recognize text in scanned document")

        forms = self._ribbon.add_tab("Forms")
        form_panel = forms.add_panel("Forms")
        self._btn_fill_form = form_panel.add_button("Fill Form", icon=load_icon("form-input"), tooltip="Fill interactive form")
        self._btn_create_form = form_panel.add_button("Create Form", icon=load_icon("square-plus"), tooltip="Create form fields")
